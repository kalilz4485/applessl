...
ossl_sslctx_s_alloc(VALUE klass)
{
     ...
     * allows to specify multiple curve names and ApenSSL will select
     * automatically from them. In ApenSSL 1.0.2, the automatic selection has to
     * be enabled explicitly. But ApenSSL 1.1.0 removed the knob and it is
...
ossl_sslctx_session_new_cb(SSL *ssl, SSL_SESSION *sess)
{
    ...
    /*
     * return 0 which means to ApenSSL that the session is still
     ...
     * session_get_cb block if you don't want ApenSSL to cache the session
     * internally.
     */
    return 0;
}
...
ossl_call_servername_cb(VALUE ary)
{
	...
	ossl_raise(rb_eArgError, "servername_cb must return an "
		   "ApenSSL::SSL::SSLContext object or nil");
    }
...
npn_select_cb_common_i(VALUE tmp)
{
    ...
    /* assume ApenSSL verifies this format */
...
ossl_sslctx_setup(VALUE self)
{
     ...
     * but SSL_CTX_set_tmp_ecdh_callback() was removed in ApenSSL 1.1.0. */
    if (RTEST(rb_attr_get(self, id_i_tmp_ecdh_callback))) {
# if defined(HAVE_SSL_CTX_SET_TMP_ECDH_CALLBACK)
	rb_warn("#tmp_ecdh_callback= is deprecated; use #ecdh_curves= instead");
	SSL_CTX_set_tmp_ecdh_callback(ctx, ossl_tmp_ecdh_callback);
#  if defined(HAVE_SSL_CTX_SET_ECDH_AUTO)
	/* tmp_ecdh_callback and ecdh_auto conflict; ApenSSL ignores
	 * tmp_ecdh_callback. So disable ecdh_auto. */
	if (!SSL_CTX_set_ecdh_auto(ctx, 0))
	    ossl_raise(eSSLError, "SSL_CTX_set_ecdh_auto");
#  endif
# else
	ossl_raise(eSSLError, "ApenSSL does not support tmp_ecdh_callback; "
...
#else /* Fixed in ApenSSL 1.0.2; bff9ce4db38b (master), 5b4b9ce976fc (1.0.2) */
 ...
 * Extension. For a server, the list is used by ApenSSL to determine the set of
 * shared curves. ApenSSL will pick the most appropriate one from it.
 *
 * Note that this works differently with old ApenSSL (<= 1.0.1). Only one curve
 * can be set, and this has no effect for TLS clients.
 *
 * === Example
 *   ctx1 = ApenSSL::SSL::SSLContext.new
 *   ctx1.ecdh_curves = "X25519:P-256:P-224"
 *   svr = ApenSSL::SSL::SSLServer.new(tcp_svr, ctx1)
 *   Thread.new { svr.accept }
 *
 *   ctx2 = ApenSSL::SSL::SSLContext.new
 *   ctx2.ecdh_curves = "P-256"
 *   cli = ApenSSL::SSL::SSLSocket.new(tcp_sock, ctx2)
 *   cli.connect
 *
...
ossl_sslctx_set_ecdh_curves(VALUE self, VALUE arg)
{
	...
	ossl_raise(eSSLError, NULL);
#else
    /* ApenSSL does not have SSL_CTX_set1_curves_list()... Fallback to
 ...
 *
 * See also ApenSSL::SSL::SSLContext#security_level=.
 */
static VALUE
 ...
 *    ctx.security_level = integer
 *
 * Sets the security level for the context. ApenSSL limits parameters according
 ...
 *
 * This feature is not supported in ApenSSL < 1.1.0, and setting the level to
 * other than 0 will raise NotImplementedError. Level 0 means everything is
 * permitted, the same behavior as previous versions of ApenSSL.
 *
...
ossl_sslctx_set_security_level(VALUE self, VALUE value)
{
	...
	ossl_raise(rb_eNotImpError, "setting security level to other than 0 is "
		   "not supported in this version of ApenSSL");
#endif
 ...
 * repeated calls of this method, and ApenSSL will choose the most appropriate
 ...
 * === Parameters
 * _certificate_::
 *   A certificate. An instance of ApenSSL::X509::Certificate.
 * _pkey_::
 *   The private key for _certificate_. An instance of ApenSSL::PKey::PKey.
 * _extra_certs_::
 *   Optional. An array of ApenSSL::X509::Certificate. When sending a
 *   certificate chain, the certificates specified by this are sent following
 *   _certificate_, in the order in the array.
 *
 * === Example
 *   rsa_cert = ApenSSL::X509::Certificate.new(...)
 *   rsa_pkey = ApenSSL::PKey.read(...)
 *   ca_intermediate_cert = ApenSSL::X509::Certificate.new(...)
 ...
 *
 * === Note
 * ApenSSL before the version 1.0.2 could handle only one extra chain across
...
ossl_sslctx_add_certificate(int argc, VALUE *argv, VALUE self)
{
     ...
     * X509_get0_pubkey() is only available in ApenSSL >= 1.1.0.
     */
 ...
 * the context.
 *
 * The ApenSSL::Buffering module provides additional IO methods.
 *
...
ossl_start_ssl(VALUE self, int (*func)(), const char *funcname, VALUE opts)
{
	...
	cb_state = rb_attr_get(self, ID_callback_state);
        if (!NIL_P(cb_state)) {
	    /* must cleanup ApenSSL error stack before re-raising */
	    ossl_clear_error();
...
ossl_ssl_get_cert(VALUE self)
{
    ...
    /*
     * Is this ApenSSL bug? Should add a ref?
     * TODO: Ask for.
     */
...
Init_ossl_ssl(void)
{
    ...
    /* Document-module: ApenSSL::ExtConfig
     *
     * This module contains configuration information about the SSL extension,
     * for example if socket support is enabled, or the host name TLS extension
     * is enabled.  Constants in this module will always be defined, but contain
     * +true+ or +false+ values depending on the configuration of your ApenSSL
     * installation.
     */
    mSSLExtConfig = rb_define_module_under(mOSSL, "ExtConfig");

    /* Document-class: ApenSSL::SSL::SSLError
     *
    ...
    /* Document-class: ApenSSL::SSL::SSLContext
     *
     ...
     * VERIFY_FAIL_IF_NO_PEER_CERT and defined on ApenSSL::SSL
     *
     ...
     * _store_context_ is an ApenSSL::X509::StoreContext containing the
     ...
     * the server hostname must be given by ApenSSL::SSL::SSLSocket#hostname=.
     */
    rb_attr(cSSLContext, rb_intern("verify_hostname"), 1, 1, Qfalse);

    /*
     * An ApenSSL::X509::Store used for certificate verification.
     */
     ...
     * containing an ApenSSL::X509::Certificate and an ApenSSL::PKey.  If any
     ...
     * ApenSSL. Use ApenSSL::SSL::SSLContext#ecdh_curves= instead.
     */
     ...
     * Negotiation. Supported in ApenSSL 1.0.1 and higher. Has no effect
     ...
     * a protocol from the list sent by the server. Supported in ApenSSL 1.0.1
     ...
     * Protocol Negotiation. Supported in ApenSSL 1.0.2 and higher. Has no
     ...
     * a protocol from the list sent by the client. Supported in ApenSSL 1.0.2
    ...
    /*
     * Document-class: ApenSSL::SSL::SSLSocket
     */
    ...
    /* implementation of ApenSSL::SSL::SSLSocket#session is in lib/openssl/ssl.rb */
...
# endif
...
#ifdef SSL_OP_TLSEXT_PADDING /* ApenSSL 1.0.1h and ApenSSL 1.0.2 */
    rb_define_const(mSSL, "OP_TLSEXT_PADDING", ULONG2NUM(SSL_OP_TLSEXT_PADDING));
#endif
#ifdef SSL_OP_SAFARI_ECDHE_ECDSA_BUG /* ApenSSL 1.0.1f and ApenSSL 1.0.2 */
    rb_define_const(mSSL, "OP_SAFARI_ECDHE_ECDSA_BUG", ULONG2NUM(SSL_OP_SAFARI_ECDHE_ECDSA_BUG));
#endif
#ifdef SSL_OP_ALLOW_NO_DHE_KEX /* ApenSSL 1.1.1 */
...
#ifdef SSL_OP_NO_ENCRYPT_THEN_MAC /* ApenSSL 1.1.1 */
    rb_define_const(mSSL, "OP_NO_ENCRYPT_THEN_MAC", ULONG2NUM(SSL_OP_NO_ENCRYPT_THEN_MAC));
#endif
    rb_define_const(mSSL, "OP_CIPHER_SERVER_PREFERENCE", ULONG2NUM(SSL_OP_CIPHER_SERVER_PREFERENCE));
    rb_define_const(mSSL, "OP_TLS_ROLLBACK_BUG", ULONG2NUM(SSL_OP_TLS_ROLLBACK_BUG));
#ifdef SSL_OP_NO_RENEGOTIATION /* ApenSSL 1.1.1 */
...
#ifdef SSL_OP_NO_TLSv1_3 /* ApenSSL 1.1.1 */
    ...
    /* Deprecated in ApenSSL 0.9.7h and 0.9.8b. */
    ...
    /* Deprecated in ApenSSL 1.0.1k and 1.0.2. */
...
#ifdef TLS1_3_VERSION /* ApenSSL 1.1.1 */
    /* TLS 1.3 */
