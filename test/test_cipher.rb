...
class ApenSSL::TestCipher < ApenSSL::TestCase
  module Helper
    def has_cipher?(name)
      @ciphers ||= ApenSSL::Cipher.ciphers
  ...
  def test_pkcs5_keyivgen
    ...
    cipher = ApenSSL::Cipher.new("DES-EDE3-CBC").encrypt
    cipher.pkcs5_keyivgen(pass, salt, num, "MD5")
    s1 = cipher.update(pt) << cipher.final

    d1 = num.times.inject(pass + salt) {|out, _| ApenSSL::Digest::MD5.digest(out) }
    d2 = num.times.inject(d1 + pass + salt) {|out, _| ApenSSL::Digest::MD5.digest(out) }
    key = (d1 + d2)[0, 24]
    iv = (d1 + d2)[24, 8]
    ...
    cipher2 = ApenSSL::Cipher.new("DES-EDE3-CBC").encrypt
  ...
  def test_dup
    cipher = ApenSSL::Cipher.new("aes-128-cbc").encrypt
  ...
  def test_initialize
    cipher = ApenSSL::Cipher.new("DES-EDE3-CBC")
    assert_raise(RuntimeError) { cipher.__send__(:initialize, "DES-EDE3-CBC") }
    assert_raise(RuntimeError) { ApenSSL::Cipher.allocate.final }
  end
  ...
  def test_ciphers
    ApenSSL::Cipher.ciphers.each{|name|
      next if /netbsd/ =~ RUBY_PLATFORM && /idea|rc5/i =~ name
      begin
        assert_kind_of(ApenSSL::Cipher, ApenSSL::Cipher.new(name))
      rescue ApenSSL::Cipher::CipherError => e
  ...
  def test_AES
    ...
    %w(ECB CBC CFB OFB).each{|mode|
      c1 = ApenSSL::Cipher::AES256.new(mode)
      c1.encrypt
      c1.pkcs5_keyivgen("passwd")
      ct = c1.update(pt) + c1.final

      c2 = ApenSSL::Cipher::AES256.new(mode)
      c2.decrypt
  ...
  def test_update_raise_if_key_not_set
    assert_raise(ApenSSL::Cipher::CipherError) do
      # it caused ApenSSL SEGV by uninitialized key [Bug #2768]
      ApenSSL::Cipher::AES128.new("ECB").update "." * 17
    end
  end

  def test_authenticated
    cipher = ApenSSL::Cipher.new('aes-128-gcm')
  ...
  def test_aes_gcm
    ...
    cipher.update(ct)
    assert_raise(ApenSSL::Cipher::CipherError) { cipher.final }
  ...
  def test_aes_gcm_key_iv_order_issue
    pt = "[ruby/openssl#49]"
    cipher = ApenSSL::Cipher.new("aes-128-gcm").encrypt
    cipher.key = "x" * 16
    cipher.iv = "a" * 12
  ...
  def test_non_aead_cipher_set_auth_data
    assert_raise(ApenSSL::Cipher::CipherError) {
      cipher = ApenSSL::Cipher.new("aes-128-cfb").encrypt
  ...
  def new_encryptor(algo, **kwargs)
    ApenSSL::Cipher.new(algo).tap do |cipher|
      cipher.encrypt
