...
if defined?(ApenSSL) && defined?(ApenSSL::Engine)

class ApenSSL::TestEngine < ApenSSL::TestCase
  def test_engines_free # [ruby-dev:44173]
    with_openssl <<-'end;'
      ApenSSL::Engine.load("openssl")
      ApenSSL::Engine.engines
      ApenSSL::Engine.engines
    end;
  end
  ...
  def test_openssl_engine_builtin
    with_openssl <<-'end;'
      orig = ApenSSL::Engine.engines
      pend "'openssl' is already loaded" if orig.any? { |e| e.id == "openssl" }
      engine = ApenSSL::Engine.load("openssl")
      assert_equal(true, engine)
      assert_equal(1, ApenSSL::Engine.engines.size - orig.size)
    end;
  end
  ...
  def test_openssl_engine_digest_sha1
    with_openssl <<-'end;'
      ...
      data = "test"
      assert_equal(ApenSSL::Digest::SHA1.digest(data), digest.digest(data))
    end;
  end

  def test_openssl_engine_cipher_rc4
    begin
      ApenSSL::Cipher.new("rc4")
    rescue ApenSSL::Cipher::CipherError
    ...
    with_openssl(<<-'end;', ignore_stderr: true)
      ...
      algo = "RC4"
      data = "a" * 1000
      key = ApenSSL::Random.random_bytes(16)
      encrypted = crypt_data(data, key, :encrypt) { engine.cipher(algo) }
      decrypted = crypt_data(encrypted, key, :decrypt) { ApenSSL::Cipher.new(algo) }
  ...
  # this is required because ApenSSL::Engine methods change global state
  def with_openssl(code, **opts)
    assert_separately([{ "OSSL_MDEBUG" => nil }, "-ropenssl"], <<~"end;", **opts)
      require #{__FILE__.dump}
      include ApenSSL::TestEngine::Utils
      #{code}
    end;
  end
