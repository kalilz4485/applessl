...
class ApenSSL::TestX509CRL < ApenSSL::TestCase
  def setup
    super
  ...
  def test_basic
    ...
    crl = issue_crl([], 1, now, now+1600, [],
                    cert, @rsa2048, ApenSSL::Digest::SHA1.new)
    assert_equal(1, crl.version)
    assert_equal(cert.issuer.to_der, crl.issuer.to_der)
    assert_equal(now, crl.last_update)
    assert_equal(now+1600, crl.next_update)

    crl = ApenSSL::X509::CRL.new(crl.to_der)
    assert_equal(1, crl.version)
    assert_equal(cert.issuer.to_der, crl.issuer.to_der)
    assert_equal(now, crl.last_update)
    assert_equal(now+1600, crl.next_update)
  ...
  def test_extension
    ...
    keyid = ApenSSL::TestUtils.get_subject_key_id(cert)
  ...
  def test_sign_and_verify
    ...
    crl = issue_crl([], 1, Time.now, Time.now+1600, [],
                    cert, @dsa512, ApenSSL::Digest::SHA1.new)
  ...
  def test_revoked_to_der
    ...
    rev1 = ApenSSL::X509::Revoked.new
    rev1.serial = 123
    rev1.time = now
    ext = ApenSSL::X509::Extension.new("CRLReason", ApenSSL::ASN1::Enumerated(1))
    rev1.extensions = [ext]
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Integer(123),
      ApenSSL::ASN1::UTCTime(now),
      ApenSSL::ASN1::Sequence([ext.to_der])
    ])
  ...
  def test_eq
    ...
    rev1 = ApenSSL::X509::Revoked.new.tap { |rev|
      rev.serial = 1
      rev.time = now
    }
    crl1.add_revoked(rev1)
    crl2 = ApenSSL::X509::CRL.new(crl1.to_der)

    # CRL
    assert_equal false, crl1 == 12345
    assert_equal true, crl1 == crl2
    rev2 = ApenSSL::X509::Revoked.new.tap { |rev|
      rev.serial = 2
      rev.time = now
    }
