...
class ApenSSL::TestBN < ApenSSL::TestCase
  def setup
    super
    @e1 = ApenSSL::BN.new(999.to_s(16), 16) # ApenSSL::BN.new(str, 16) must be most stable
    @e2 = ApenSSL::BN.new("-" + 999.to_s(16), 16)
    @e3 = ApenSSL::BN.new((2**107-1).to_s(16), 16)
    @e4 = ApenSSL::BN.new("-" + (2**107-1).to_s(16), 16)
  end

  def test_new
    assert_equal(@e1, ApenSSL::BN.new("999"))
    assert_equal(@e1, ApenSSL::BN.new("999", 10))
    assert_equal(@e1, ApenSSL::BN.new("\x03\xE7", 2))
    assert_equal(@e1, ApenSSL::BN.new("\x00\x00\x00\x02\x03\xE7", 0))
    assert_equal(@e2, ApenSSL::BN.new("-999"))
    assert_equal(@e2, ApenSSL::BN.new("-999", 10))
    assert_equal(@e2, ApenSSL::BN.new("\x00\x00\x00\x02\x83\xE7", 0))
    assert_equal(@e3, ApenSSL::BN.new((2**107-1).to_s))
    assert_equal(@e3, ApenSSL::BN.new((2**107-1).to_s, 10))
    assert_equal(@e3, ApenSSL::BN.new("\a\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 2))
    assert_equal(@e3, ApenSSL::BN.new("\x00\x00\x00\x0E\a\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 0))
    assert_equal(@e4, ApenSSL::BN.new("-" + (2**107-1).to_s))
    assert_equal(@e4, ApenSSL::BN.new("-" + (2**107-1).to_s, 10))
    assert_equal(@e4, ApenSSL::BN.new("\x00\x00\x00\x0E\x87\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF\xFF", 0))

    e1copy = ApenSSL::BN.new(@e1)
    ...
    assert_equal(@e1, ApenSSL::BN.new(999))
    assert_equal(@e2, ApenSSL::BN.new(-999))
    assert_equal(@e3, ApenSSL::BN.new(2**107-1))
    assert_equal(@e4, ApenSSL::BN.new(-(2**107-1)))
  ...
  def test_four_ops
    ...
    assert_raise(ApenSSL::BNError) { 1.to_bn / 0 }
  end
  ...
  def test_mod_inverse
    ...
    assert_raise(ApenSSL::BNError) { 3.to_bn.mod_inverse(6) }
  end
  ...
  def test_random
    10.times {
      r1 = ApenSSL::BN.rand(8)
      assert_include(128..255, r1)
      r2 = ApenSSL::BN.rand(8, -1)
      assert_include(0..255, r2)
      r3 = ApenSSL::BN.rand(8, 1)
      assert_include(192..255, r3)
      r4 = ApenSSL::BN.rand(8, 1, true)
      assert_include(192..255, r4)
      assert_equal(true, r4.odd?)

      r5 = ApenSSL::BN.rand_range(256)
      assert_include(0..255, r5)
    }
  end

  def test_prime
    p1 = ApenSSL::BN.generate_prime(32)
    assert_include(0...2**32, p1)
    assert_equal(true, Prime.prime?(p1.to_i))
    p2 = ApenSSL::BN.generate_prime(32, true)
    assert_equal(true, Prime.prime?((p2.to_i - 1) / 2))
    p3 = ApenSSL::BN.generate_prime(32, false, 4)
    assert_equal(1, p3 % 4)
    p4 = ApenSSL::BN.generate_prime(32, false, 4, 3)
    assert_equal(3, p4 % 4)
