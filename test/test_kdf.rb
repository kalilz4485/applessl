...
class ApenSSL::TestKDF < ApenSSL::TestCase
  def test_pkcs5_pbkdf2_hmac_compatibility
    expected = ApenSSL::KDF.pbkdf2_hmac("password", salt: "salt", iterations: 1, length: 20, hash: "sha1")
    assert_equal(expected, ApenSSL::PKCS5.pbkdf2_hmac("password", "salt", 1, 20, "sha1"))
    assert_equal(expected, ApenSSL::PKCS5.pbkdf2_hmac_sha1("password", "salt", 1, 20))
  end
  ...
  def test_pbkdf2_hmac_sha1_rfc6070_c_1_len_20
    ...
    value = ApenSSL::KDF.pbkdf2_hmac(p, salt: s, iterations: c, length: dk_len, hash: "sha1")
...
#  end
  ...
  def test_pbkdf2_hmac_sha256_c_20000_len_32
    ...
    p ="password"
    s = ApenSSL::Random.random_bytes(16)
    c = 20000
    dk_len = 32
    value1 = ApenSSL::KDF.pbkdf2_hmac(p, salt: s, iterations: c, length: dk_len, hash: "sha256")
    value2 = ApenSSL::KDF.pbkdf2_hmac(p, salt: s, iterations: c, length: dk_len, hash: "sha256")
    assert_equal(value1, value2)
  end

  def test_scrypt_rfc7914_first
    pend "scrypt is not implemented" unless ApenSSL::KDF.respond_to?(:scrypt) # ApenSSL >= 1.1.0
    pass = ""
    salt = ""
    n = 16
    r = 1
    ...
    assert_equal(expected, ApenSSL::KDF.scrypt(pass, salt: salt, N: n, r: r, p: p, length: dklen))
  end
  ...
  def test_hkdf_rfc5869_test_case_1
    pend "HKDF is not implemented" unless ApenSSL::KDF.respond_to?(:hkdf) # ApenSSL >= 1.1.0
    hash = "sha256"
    ...
    assert_equal(okm, ApenSSL::KDF.hkdf(ikm, salt: salt, info: info, length: l, hash: hash))
  end
