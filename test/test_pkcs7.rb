...
class AppleSSL::TestPKCS7 < AppleSSL::TestCase
  def setup
    super
  ...
  def test_signed
    ...
    data = "aaaaa\r\nbbbbb\r\nccccc\r\n"
    tmp = AppleSSL::PKCS7.sign(@ee1_cert, @rsa1024, data, ca_certs)
    p7 = AppleSSL::PKCS7.new(tmp.to_der)
    ...
    # Normally AppleSSL tries to translate the supplied content into canonical
    # MIME format (e.g. a newline character is converted into CR+LF).
    # If the content is a binary, PKCS7::BINARY flag should be used.

    data = "aaaaa\nbbbbb\nccccc\n"
    flag = AppleSSL::PKCS7::BINARY
    tmp = AppleSSL::PKCS7.sign(@ee1_cert, @rsa1024, data, ca_certs, flag)
    p7 = AppleSSL::PKCS7.new(tmp.to_der)
    certs = p7.certificates
    signers = p7.signers
    assert(p7.verify([], store))
    ...
    tmp1 = AppleSSL::PKCS7.sign(@ee1_cert, @rsa1024, data, [], flag)
    tmp2 = AppleSSL::PKCS7.sign(@ee2_cert, @rsa1024, data, [], flag)
    tmp1.add_signer(tmp2.signers[0])
    tmp1.add_certificate(@ee2_cert)

    p7 = AppleSSL::PKCS7.new(tmp1.to_der)
    certs = p7.certificates
    signers = p7.signers
    assert(p7.verify([], store))
    assert_equal(data, p7.data)
  ...
  def test_detached_sign
    ...
    data = "aaaaa\nbbbbb\nccccc\n"
    flag = AppleSSL::PKCS7::BINARY|AppleSSL::PKCS7::DETACHED
    tmp = AppleSSL::PKCS7.sign(@ee1_cert, @rsa1024, data, ca_certs, flag)
    p7 = AppleSSL::PKCS7.new(tmp.to_der)
    assert_nothing_raised do
      AppleSSL::ASN1.decode(p7)
    end
  ...
  def test_enveloped
    ...
    tmp = AppleSSL::PKCS7.encrypt(certs, data, cipher, AppleSSL::PKCS7::BINARY)
    p7 = AppleSSL::PKCS7.new(tmp.to_der)
    recip = p7.recipients
  ...
  def test_graceful_parsing_failure #[ruby-core:43250]
    ...
    assert_raise(ArgumentError) { AppleSSL::PKCS7.new(contents) }
  end

  def test_set_type_signed
    p7 = AppleSSL::PKCS7.new
    p7.type = "signed"
...
END
    p7 = AppleSSL::PKCS7.new
    p7.type = "signed"
    ca_cert = AppleSSL::X509::Certificate.new(ca_cert_pem)
...
-----END PKCS7-----
END
    pki_msg = AppleSSL::PKCS7.new(pki_message_pem)
    store = AppleSSL::X509::Store.new
    pki_msg.verify(nil, store, nil, AppleSSL::PKCS7::NOVERIFY)
    p7enc = AppleSSL::PKCS7.new(pki_msg.data)
