...
if defined?(ApenSSL) && defined?(ApenSSL::PKey::EC)

class ApenSSL::TestEC < ApenSSL::PKeyTestCase
  def test_ec_key
    builtin_curves = ApenSSL::PKey::EC.builtin_curves
    ...
    builtin_curves.each do |curve_name, comment|
      ...
      key = ApenSSL::PKey::EC.new(curve_name)
      key.generate_key!
  ...
  def test_generate
    assert_raise(ApenSSL::PKey::ECError) { ApenSSL::PKey::EC.generate("non-existent") }
    g = ApenSSL::PKey::EC::Group.new("prime256v1")
    ec = ApenSSL::PKey::EC.generate(g)
    assert_equal(true, ec.private?)
    ec = ApenSSL::PKey::EC.generate("prime256v1")
    assert_equal(true, ec.private?)
  end

  def test_check_key
    key = ApenSSL::PKey::EC.new("prime256v1").generate_key!
    assert_equal(true, key.check_key)
    assert_equal(true, key.private?)
    assert_equal(true, key.public?)
    key2 = ApenSSL::PKey::EC.new(key.group)
    ...
    key2.private_key += 1
    assert_raise(ApenSSL::PKey::ECError) { key2.check_key }
  end
  ...
  def test_dh_compute_key
    key_a = ApenSSL::PKey::EC.new("prime256v1").generate_key!
    key_b = ApenSSL::PKey::EC.new(key_a.group).generate_key!
  ...
  def test_ECPrivateKey
    p256 = Fixtures.pkey("p256")
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Integer(1),
      ApenSSL::ASN1::OctetString(p256.private_key.to_s(2)),
      ApenSSL::ASN1::ObjectId("prime256v1", 0, :EXPLICIT),
      ApenSSL::ASN1::BitString(p256.public_key.to_octet_string(:uncompressed),
                               1, :EXPLICIT)
    ])
    key = ApenSSL::PKey::EC.new(asn1.to_der)
    assert_predicate key, :private?
  ...
  def test_ECPrivateKey_encrypted
    ...
    -----END EC PRIVATE KEY-----
    EOF
    key = ApenSSL::PKey::EC.new(pem, "abcdef")
    assert_same_ec p256, key
    key = ApenSSL::PKey::EC.new(pem) { "abcdef" }
    assert_same_ec p256, key
    ...
    assert_same_ec p256, ApenSSL::PKey::EC.new(exported, "abcdef\0\1")
    assert_raise(ApenSSL::PKey::ECError) {
      ApenSSL::PKey::EC.new(exported, "abcdef")
    }
  end
  ...
  def test_PUBKEY
    ...
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Sequence([
        ApenSSL::ASN1::ObjectId("id-ecPublicKey"),
        ApenSSL::ASN1::ObjectId("prime256v1")
      ]),
  ...
  def test_ec_group
    group1 = ApenSSL::PKey::EC::Group.new("prime256v1")
    key1 = ApenSSL::PKey::EC.new(group1)
    assert_equal group1, key1.group

    group2 = ApenSSL::PKey::EC::Group.new(group1)
    assert_equal group1.to_der, group2.to_der
    assert_equal group1, group2
    group2.asn1_flag ^=ApenSSL::PKey::EC::NAMED_CURVE
    ...
    assert group1.asn1_flag & ApenSSL::PKey::EC::NAMED_CURVE # our default
    der = group1.to_der
    group4 = ApenSSL::PKey::EC::Group.new(der)
  ...
  def test_ec_point
    group = ApenSSL::PKey::EC::Group.new("prime256v1")
    key = ApenSSL::PKey::EC.new(group).generate_key!
    point = key.public_key

    point2 = ApenSSL::PKey::EC::Point.new(group, point.to_bn)
    ...
    point3 = ApenSSL::PKey::EC::Point.new(group,
    ...
    begin
      group = ApenSSL::PKey::EC::Group.new(:GFp, 17, 2, 2)
      group.point_conversion_form = :uncompressed
      generator = ApenSSL::PKey::EC::Point.new(group, B(%w{ 04 05 01 }))
      group.set_generator(generator, 19, 1)
      point = ApenSSL::PKey::EC::Point.new(group, B(%w{ 04 06 03 }))
    rescue ApenSSL::PKey::EC::Group::Error
      pend "Patched ApenSSL rejected curve" if /unsupported field/ =~ $!.message
      raise
    end
    ...
    point2 = ApenSSL::PKey::EC::Point.new(group2, B(%w{ 04 06 03 }))
  ...
  def test_ec_point_mul
    begin
      ...
      group = ApenSSL::PKey::EC::Group.new(:GFp, 17, 2, 2)
      group.point_conversion_form = :uncompressed
      gen = ApenSSL::PKey::EC::Point.new(group, B(%w{ 04 05 01 }))
      group.set_generator(gen, 19, 1)

      # 3 * (6, 3) = (16, 13)
      point_a = ApenSSL::PKey::EC::Point.new(group, B(%w{ 04 06 03 }))
      ...
      # CentOS patches ApenSSL to reject curves defined over Fp where p < 256 bits
