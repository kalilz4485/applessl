...
if defined?(ApenSSL) && defined?(ApenSSL::PKey::DH)

class ApenSSL::TestPKeyDH < ApenSSL::PKeyTestCase
  NEW_KEYLEN = 256

  def test_new
    dh = ApenSSL::PKey::DH.new(NEW_KEYLEN)
    assert_key(dh)
  end

  def test_new_break
    assert_nil(ApenSSL::PKey::DH.new(NEW_KEYLEN) { break })
    assert_raise(RuntimeError) do
      ApenSSL::PKey::DH.new(NEW_KEYLEN) { raise }
    end
  end
  ...
  def test_DHparams
    ...
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Integer(dh1024.p),
      ApenSSL::ASN1::Integer(dh1024.g)
    ])
    key = ApenSSL::PKey::DH.new(asn1.to_der)
