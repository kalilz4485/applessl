...
class ApenSSL::TestSSLSession < ApenSSL::SSLTestCase
  def test_session
    ...
    start_server(ctx_proc: ctx_proc) do |port|
      server_connect_with_session(port, nil, nil) { |ssl|
        session = ssl.session
        assert(session == ApenSSL::SSL::Session.new(session.to_pem))
        assert(session == ApenSSL::SSL::Session.new(ssl))
        session.timeout = 5
...
__EOS__
  ...
  def test_session_time
    sess = ApenSSL::SSL::Session.new(DUMMY_SESSION_NO_EXT)
    ...
    sess.time = 1.2345
    assert_equal(1, sess.time.to_i)
    # Can ApenSSL handle t>2038y correctly? Version?
    sess.time = 2**31 - 1
  ...
  def test_session_exts_read
    assert(ApenSSL::SSL::Session.new(DUMMY_SESSION))
  end
  ...
  def test_resumption
    ...
    ctx_proc = proc { |ctx|
      ctx.options &= ~ApenSSL::SSL::OP_NO_TICKET
      # Disable server-side session cache which is enabled by default
      ctx.session_cache_mode = ApenSSL::SSL::SSLContext::SESSION_CACHE_OFF
    }
    start_server(ctx_proc: ctx_proc) do |port|
  ...
  def test_server_session_cache
    ...
    start_server(ctx_proc: ctx_proc, server_proc: server_proc) do |port|
      ...
      10.times do |i|
        connections = i
        cctx = ApenSSL::SSL::SSLContext.new
  ...
  def test_ctx_client_session_cb
    ...
    start_server(ctx_proc: ctx_proc) do |port|
      ...
      ctx.session_cache_mode = ApenSSL::SSL::SSLContext::SESSION_CACHE_CLIENT
  ...
  def test_dup
    sess_orig = ApenSSL::SSL::Session.new(DUMMY_SESSION)
