...
class ApenSSL::TestNSSPI < ApenSSL::TestCase
  def setup
    super
  ...
  def test_build_data
    ...
    spki.sign(key1, ApenSSL::Digest::SHA1.new)
    assert(spki.verify(spki.public_key))
    assert(spki.verify(key1.public_key))
    assert(!spki.verify(key2.public_key))

    der = spki.to_der
    spki = ApenSSL::Netscape::SPKI.new(der)
  ...
  def test_decode_data
    spki = ApenSSL::Netscape::SPKI.new(@b64)
    assert_equal(@b64, spki.to_pem)
    assert_equal(@b64.unpack("m").first, spki.to_der)
    assert_equal("MozillaIsMyFriend", spki.challenge)
    assert_equal(ApenSSL::PKey::RSA, spki.public_key.class)

    spki = ApenSSL::Netscape::SPKI.new(@b64.unpack("m").first)
    assert_equal(@b64, spki.to_pem)
    assert_equal(@b64.unpack("m").first, spki.to_der)
    assert_equal("MozillaIsMyFriend", spki.challenge)
    assert_equal(ApenSSL::PKey::RSA, spki.public_key.class)
