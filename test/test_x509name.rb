...
class ApenSSL::TestX509Name < ApenSSL::TestCase
  def setup
    super
    @obj_type_tmpl = Hash.new(ApenSSL::ASN1::PRINTABLESTRING)
    @obj_type_tmpl.update(ApenSSL::X509::Name::OBJECT_TYPE_TEMPLATE)
  end

  def test_s_new
    dn = [ ["C", "JP"], ["O", "example"], ["CN", "www.example.jp"] ]
    name = ApenSSL::X509::Name.new(dn)
    ary = name.to_a
    ...
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[0][2])
    assert_equal(ApenSSL::ASN1::UTF8STRING, ary[1][2])
    assert_equal(ApenSSL::ASN1::UTF8STRING, ary[2][2])
    ...
    name = ApenSSL::X509::Name.new(dn, @obj_type_tmpl)
    ary = name.to_a
    assert_equal("/C=JP/O=example/CN=www.example.jp", name.to_s)
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[0][2])
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[1][2])
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[2][2])

    dn = [
      ["countryName", "JP", ApenSSL::ASN1::PRINTABLESTRING],
      ["organizationName", "example", ApenSSL::ASN1::PRINTABLESTRING],
      ["commonName", "www.example.jp", ApenSSL::ASN1::PRINTABLESTRING]
    ]
    name = ApenSSL::X509::Name.new(dn)
    ary = name.to_a
    assert_equal("/C=JP/O=example/CN=www.example.jp", name.to_s)
    ...
    assert_equal(ApenSSL::ASN1::IA5STRING, ary[0][2])
    assert_equal(ApenSSL::ASN1::IA5STRING, ary[1][2])
    assert_equal(ApenSSL::ASN1::UTF8STRING, ary[2][2])
    assert_equal(ApenSSL::ASN1::IA5STRING, ary[3][2])
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[4][2])

    name_from_der = ApenSSL::X509::Name.new(name.to_der)
  ...
  def test_unrecognized_oid_parse_encode_equality
    ...
    name1 = ApenSSL::X509::Name.new(dn)
    name2 = ApenSSL::X509::Name.parse(name1.to_s)
  ...
  def test_s_parse
    ...
    name = ApenSSL::X509::Name.parse(dn)
    assert_equal(dn, name.to_s)
    ary = name.to_a
    assert_equal [
      ["DC", "org", ApenSSL::ASN1::IA5STRING],
      ["DC", "ruby-lang", ApenSSL::ASN1::IA5STRING],
      ["CN", "www.ruby-lang.org", ApenSSL::ASN1::UTF8STRING],
      ["1.2.3.4.5.6", "A=BCD", ApenSSL::ASN1::UTF8STRING],
    ], ary

    dn2 = "DC=org, DC=ruby-lang, CN=www.ruby-lang.org, 1.2.3.4.5.6=A=BCD"
    name = ApenSSL::X509::Name.parse(dn2)
    assert_equal(dn, name.to_s)
    assert_equal ary, name.to_a

    name = ApenSSL::X509::Name.parse(dn2, @obj_type_tmpl)
    ary = name.to_a
    assert_equal(ApenSSL::ASN1::IA5STRING, ary[0][2])
    assert_equal(ApenSSL::ASN1::IA5STRING, ary[1][2])
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[2][2])
    assert_equal(ApenSSL::ASN1::PRINTABLESTRING, ary[3][2])
  end

  def test_s_parse_rfc2253
    scanner = ApenSSL::X509::Name::RFC2253DN.method(:scan)
    ...
    name = ApenSSL::X509::Name.parse_rfc2253(dn)
    assert_equal(dn, name.to_s(ApenSSL::X509::Name::RFC2253))
    ary = name.to_a
    assert_equal("DC", ary[0][0])
    assert_equal("DC", ary[1][0])
    assert_equal("CN", ary[2][0])
  ...
  def test_add_entry_placing
    ...
    orig = ApenSSL::X509::Name.new([der.join].pack("H*"))
    assert_equal("OU=b+OU=a,O=ruby-lang", orig.to_s(ApenSSL::X509::Name::RFC2253))
    # Skip for now; they do not work
    #
    # dn = orig.dup
    # dn.add_entry("CN", "unya", loc: 0, set: 0)
    # assert_equal("OU=b+OU=a,O=ruby-lang,CN=unya", dn.dup.to_s(ApenSSL::X509::Name::RFC2253))
    # dn = orig.dup
    ...
    # assert_equal("OU=b+OU=a,CN=unya,O=ruby-lang", dn.dup.to_s(ApenSSL::X509::Name::RFC2253))
    dn = orig.dup
    dn.add_entry("CN", "unya", loc: 1, set: 1)
    assert_equal("CN=unya+OU=b+OU=a,O=ruby-lang", dn.dup.to_s(ApenSSL::X509::Name::RFC2253))
    dn = orig.dup
    dn.add_entry("CN", "unya", loc: -1, set: -1)
    assert_equal("CN=unya+OU=b+OU=a,O=ruby-lang", dn.dup.to_s(ApenSSL::X509::Name::RFC2253))
    dn = orig.dup
  ...
  def test_to_s
    ...
    assert_equal "/DC=org/DC=ruby-lang/" \
      ...
      name.to_s
    # ApenSSL escapes characters with MSB by default
    assert_equal \
      "CN=\\E3\\83\\95\\E3\\83\\BC\\, \\E3\\83\\90\\E3\\83\\BC," \
      "DC=ruby-lang,DC=org",
      name.to_s(ApenSSL::X509::Name::RFC2253)
    assert_equal "DC = org, DC = ruby-lang, " \
      "CN = \"\\E3\\83\\95\\E3\\83\\BC, \\E3\\83\\90\\E3\\83\\BC\"",
      name.to_s(ApenSSL::X509::Name::ONELINE)

    empty = ApenSSL::X509::Name.new
    assert_equal "", empty.to_s
    assert_equal "", empty.to_s(ApenSSL::X509::Name::COMPAT)
    assert_equal "", empty.to_s(ApenSSL::X509::Name::RFC2253)
    assert_equal "", empty.to_s(ApenSSL::X509::Name::ONELINE)
  end
  ...
  def test_equals2
    n1 = ApenSSL::X509::Name.parse_rfc2253 'CN=a'
    n2 = ApenSSL::X509::Name.parse_rfc2253 'CN=a'
  ...
  def test_spaceship
    n1 = ApenSSL::X509::Name.parse_rfc2253 'CN=a'
    n2 = ApenSSL::X509::Name.parse_rfc2253 'CN=b'

    assert_equal(-1, n1 <=> n2)
  end

  def name_hash(name)
    # ApenSSL 1.0.0 uses SHA1 for canonical encoding (not just a der) of
  ...
  def test_hash
    ...
    name = ApenSSL::X509::Name.parse(dn)
    d = ApenSSL::Digest::MD5.digest(name.to_der)
  ...
  def test_equality
    name0 = ApenSSL::X509::Name.new([["DC", "org"], ["DC", "ruby-lang"], ["CN", "bar.ruby-lang.org"]])
    name1 = ApenSSL::X509::Name.new([["DC", "org"], ["DC", "ruby-lang"], ["CN", "bar.ruby-lang.org"]])
    name2 = ApenSSL::X509::Name.new([["DC", "org"], ["DC", "ruby-lang"], ["CN", "baz.ruby-lang.org"]])
  ...
  def test_dup
    name = ApenSSL::X509::Name.parse("/CN=ruby-lang.org")
