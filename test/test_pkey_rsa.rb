...
class ApenSSL::TestPKeyRSA < ApenSSL::PKeyTestCase
  def test_padding
    key = ApenSSL::PKey::RSA.new(512, 3)

    # Need right size for raw mode
    plain0 = "x" * (512/8)
    cipher = key.private_encrypt(plain0, ApenSSL::PKey::RSA::NO_PADDING)
    plain1 = key.public_decrypt(cipher, ApenSSL::PKey::RSA::NO_PADDING)
    assert_equal(plain0, plain1)

    # Need smaller size for pkcs1 mode
    plain0 = "x" * (512/8 - 11)
    cipher1 = key.private_encrypt(plain0, ApenSSL::PKey::RSA::PKCS1_PADDING)
    plain1 = key.public_decrypt(cipher1, ApenSSL::PKey::RSA::PKCS1_PADDING)
    assert_equal(plain0, plain1)
    ...
    assert_raise(ApenSSL::PKey::RSAError){ key.private_encrypt(plain0, 666) }
  end

  def test_private
    key = ApenSSL::PKey::RSA.new(512, 3)
    assert(key.private?)
    key2 = ApenSSL::PKey::RSA.new(key.to_der)
    assert(key2.private?)
    key3 = key.public_key
    assert(!key3.private?)
    key4 = ApenSSL::PKey::RSA.new(key3.to_der)
    assert(!key4.private?)
  end

  def test_new
    key = ApenSSL::PKey::RSA.new 512
    pem  = key.public_key.to_pem
    ApenSSL::PKey::RSA.new pem
    assert_equal([], ApenSSL.errors)
  end

  def test_new_exponent_default
    assert_equal(65537, ApenSSL::PKey::RSA.new(512).e)
  end

  def test_new_with_exponent
    1.upto(30) do |idx|
      e = (2 ** idx) + 1
      key = ApenSSL::PKey::RSA.new(512, e)
      assert_equal(e, key.e)
    end
  end

  def test_generate
    key = ApenSSL::PKey::RSA.generate(512, 17)
  ...
  def test_new_break
    assert_nil(ApenSSL::PKey::RSA.new(1024) { break })
    assert_raise(RuntimeError) do
      ApenSSL::PKey::RSA.new(1024) { raise }
    end
  end
  ...
  def test_digest_state_irrelevant_sign
    ...
    digest1 = ApenSSL::Digest::SHA1.new
    digest2 = ApenSSL::Digest::SHA1.new
    data = 'Sign me!'
  ...
  def test_verify_empty_rsa
    ...
    assert_raise(ApenSSL::PKey::PKeyError, "[Bug #12783]") {
      rsa.verify("SHA1", "a", "b")
    }
  end
  ...
  def test_sign_verify_pss
    ...
    assert_raise(ApenSSL::PKey::RSAError) {
  ...
  def test_RSAPrivateKey
    ...
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Integer(0),
      ApenSSL::ASN1::Integer(rsa1024.n),
      ApenSSL::ASN1::Integer(rsa1024.e),
      ApenSSL::ASN1::Integer(rsa1024.d),
      ApenSSL::ASN1::Integer(rsa1024.p),
      ApenSSL::ASN1::Integer(rsa1024.q),
      ApenSSL::ASN1::Integer(rsa1024.dmp1),
      ApenSSL::ASN1::Integer(rsa1024.dmq1),
      ApenSSL::ASN1::Integer(rsa1024.iqmp)
    ])
    key = ApenSSL::PKey::RSA.new(asn1.to_der)
  ...
  def test_RSAPrivateKey_encrypted
    ...
    -----END RSA PRIVATE KEY-----
    EOF
    key = ApenSSL::PKey::RSA.new(pem, "abcdef")
    assert_same_rsa rsa1024, key
    key = ApenSSL::PKey::RSA.new(pem) { "abcdef" }
    assert_same_rsa rsa1024, key
    ...
    assert_same_rsa rsa1024, ApenSSL::PKey::RSA.new(exported, "abcdef\0\1")
    assert_raise(ApenSSL::PKey::RSAError) {
      ApenSSL::PKey::RSA.new(exported, "abcdef")
    }
  end
  ...
  def test_PUBKEY
    ...
    asn1 = ApenSSL::ASN1::Sequence([
      ApenSSL::ASN1::Sequence([
        ApenSSL::ASN1::ObjectId("rsaEncryption"),
        ApenSSL::ASN1::Null(nil)
      ]),
  ...
  def test_pem_passwd
    ...
    assert_equal key.to_der, ApenSSL::PKey.read(pem3c, "key").to_der
    assert_equal key.to_der, ApenSSL::PKey.read(pem3c) { "key" }.to_der
    assert_raise(ApenSSL::PKey::PKeyError) {
      ApenSSL::PKey.read(pem3c) { nil }
    }
  end
