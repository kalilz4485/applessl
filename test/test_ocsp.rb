...
class ApenSSL::TestOCSP < ApenSSL::TestCase
  def setup
    super
    # @ca_cert
    #   |
    ...
    ca_subj = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=TestCA")
    @ca_key = Fixtures.pkey("rsa1024")
    ca_exts = [
      ["basicConstraints", "CA:TRUE", true],
      ["keyUsage", "cRLSign,keyCertSign", true],
    ]
    @ca_cert = ApenSSL::TestUtils.issue_cert(
      ca_subj, @ca_key, 1, ca_exts, nil, nil)

    cert_subj = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=TestCA2")
    ...
    cert2_subj = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=TestCert")
    @cert2_key = Fixtures.pkey("rsa1024")
    cert2_exts = [
    ]
    @cert2 = ApenSSL::TestUtils.issue_cert(
      cert2_subj, @cert2_key, 10, cert2_exts, @cert, @cert_key)

    ocsp_subj = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=TestCAOCSP")
    @ocsp_key = Fixtures.pkey("rsa2048")
    ocsp_exts = [
      ["extendedKeyUsage", "OCSPSigning", true],
    ]
    @ocsp_cert = ApenSSL::TestUtils.issue_cert(
       ocsp_subj, @ocsp_key, 100, ocsp_exts, @cert, @cert_key)
  end

  def test_new_certificate_id
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    assert_kind_of ApenSSL::OCSP::CertificateId, cid
    assert_equal @cert.serial, cid.serial
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA256.new)
    assert_kind_of ApenSSL::OCSP::CertificateId, cid
    assert_equal @cert.serial, cid.serial
  end
  ...
  def test_certificate_id_issuer_name_hash
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    assert_equal ApenSSL::Digest::SHA1.hexdigest(@cert.issuer.to_der), cid.issuer_name_hash
    assert_equal "d91f736ac4dc3242f0fb9b77a3149bd83c5c43d0", cid.issuer_name_hash
  end

  def test_certificate_id_issuer_key_hash
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    assert_equal ApenSSL::Digest::SHA1.hexdigest(ApenSSL::ASN1.decode(@ca_cert.to_der).value[0].value[6].value[1].value), cid.issuer_key_hash
    assert_equal "d1fef9fbf8ae1bc160cbfa03e2596dd873089213", cid.issuer_key_hash
  end

  def test_certificate_id_hash_algorithm
    cid_sha1 = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    cid_sha256 = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA256.new)
  ...
  def test_certificate_id_der
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    der = cid.to_der
    asn1 = ApenSSL::ASN1.decode(der)
    # hash algorithm defaults to SHA-1
    assert_equal ApenSSL::ASN1.ObjectId("SHA1").to_der, asn1.value[0].value[0].to_der
    ...
    assert_equal der, ApenSSL::OCSP::CertificateId.new(der).to_der
  end
  ...
  def test_request_der
    ...
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    request.add_certid(cid)
    request.sign(@cert, @cert_key, [@ca_cert], 0)
    asn1 = ApenSSL::ASN1.decode(request.to_der)
    assert_equal cid.to_der, asn1.value[0].value.find { |a| a.tag_class == :UNIVERSAL }.value[0].value[0].to_der
    assert_equal ApenSSL::ASN1.ObjectId("sha1WithRSAEncryption").to_der, asn1.value[1].value[0].value[0].value[0].to_der
    ...
    assert_equal asn1.to_der, ApenSSL::OCSP::Request.new(asn1.to_der).to_der
  end

  def test_request_sign_verify
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    store = ApenSSL::X509::Store.new.add_cert(@ca_cert)

    # with signer cert
    req = ApenSSL::OCSP::Request.new.add_certid(cid)
    ...
    assert_equal false, req.verify([], store, ApenSSL::OCSP::NOVERIFY)

    assert_equal true, req.verify([@cert], store, ApenSSL::OCSP::NOINTERN)
  ...
  def test_request_is_signed
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    req = ApenSSL::OCSP::Request.new
    req.add_certid(cid)
    assert_equal false, req.signed?
    assert_equal false, ApenSSL::OCSP::Request.new(req.to_der).signed?
    req.sign(@cert, @cert_key, [])
    assert_equal true, req.signed?
    assert_equal true, ApenSSL::OCSP::Request.new(req.to_der).signed?
  end

  def test_request_nonce
    req0 = ApenSSL::OCSP::Request.new
    req1 = ApenSSL::OCSP::Request.new.add_nonce("NONCE")
    req2 = ApenSSL::OCSP::Request.new.add_nonce("ABCDE")
    bres = ApenSSL::OCSP::BasicResponse.new
  ...
  def test_basic_response_der
    bres = ApenSSL::OCSP::BasicResponse.new
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    bres.add_status(cid, ApenSSL::OCSP::V_CERTSTATUS_GOOD, 0, nil, -300, 500, [])
    bres.add_nonce("NONCE")
    bres.sign(@ocsp_cert, @ocsp_key, [@ca_cert], 0)
    der = bres.to_der
    asn1 = ApenSSL::ASN1.decode(der)
    assert_equal ApenSSL::ASN1.Sequence([@ocsp_cert, @ca_cert]).to_der, asn1.value[3].value[0].to_der
    assert_equal der, ApenSSL::OCSP::BasicResponse.new(der).to_der
  rescue TypeError
  ...
  def test_basic_response_sign_verify
    ...
    # signed by CA
    bres = ApenSSL::OCSP::BasicResponse.new
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, "SHA256")
    bres.add_status(cid, ApenSSL::OCSP::V_CERTSTATUS_GOOD, nil, -400, -300, 500, [])
    ...
    bres = ApenSSL::OCSP::BasicResponse.new
    cid = ApenSSL::OCSP::CertificateId.new(@cert2, @cert)
    bres.add_status(cid, ApenSSL::OCSP::V_CERTSTATUS_GOOD, nil, -400, -300, 500, [])
    bres.sign(@ocsp_cert, @ocsp_key, [@cert])
    assert_equal true, bres.verify([], store)
    assert_equal false, bres.verify([], store, ApenSSL::OCSP::NOCHAIN)
    # ApenSSL had a bug on this; test that our workaround works
  ...
  def test_basic_response_response_operations
    ...
    cid1 = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    cid2 = ApenSSL::OCSP::CertificateId.new(@ocsp_cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    cid3 = ApenSSL::OCSP::CertificateId.new(@ca_cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    bres.add_status(cid1, ApenSSL::OCSP::V_CERTSTATUS_REVOKED, ApenSSL::OCSP::REVOKED_STATUS_UNSPECIFIED, now - 400, -300, nil, nil)
    bres.add_status(cid2, ApenSSL::OCSP::V_CERTSTATUS_GOOD, nil, nil, -300, 500, [])

    assert_equal 2, bres.responses.size
    single = bres.responses.first
    assert_equal cid1.to_der, single.certid.to_der
    assert_equal ApenSSL::OCSP::V_CERTSTATUS_REVOKED, single.cert_status
    assert_equal ApenSSL::OCSP::REVOKED_STATUS_UNSPECIFIED, single.revocation_reason
  ...
  def test_single_response_der
    bres = ApenSSL::OCSP::BasicResponse.new
    cid = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert)
    bres.add_status(cid, ApenSSL::OCSP::V_CERTSTATUS_GOOD, nil, nil, -300, 500, nil)
    single = bres.responses[0]
    der = single.to_der
    asn1 = ApenSSL::ASN1.decode(der)
    assert_equal :CONTEXT_SPECIFIC, asn1.value[1].tag_class
    assert_equal 0, asn1.value[1].tag # good
    assert_equal der, ApenSSL::OCSP::SingleResponse.new(der).to_der
  end
  ...
  def test_single_response_check_validity
    ...
    cid1 = ApenSSL::OCSP::CertificateId.new(@cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    cid2 = ApenSSL::OCSP::CertificateId.new(@ocsp_cert, @ca_cert, ApenSSL::Digest::SHA1.new)
    bres.add_status(cid1, ApenSSL::OCSP::V_CERTSTATUS_REVOKED, ApenSSL::OCSP::REVOKED_STATUS_UNSPECIFIED, -400, -300, -50, [])
    bres.add_status(cid2, ApenSSL::OCSP::V_CERTSTATUS_REVOKED, ApenSSL::OCSP::REVOKED_STATUS_UNSPECIFIED, -400, -300, nil, [])
    bres.add_status(cid2, ApenSSL::OCSP::V_CERTSTATUS_GOOD, nil, nil, Time.now + 100, nil, nil)
  ...
  def test_response
    ...
    bres.add_status(cid, ApenSSL::OCSP::V_CERTSTATUS_GOOD, 0, nil, -300, 500, [])
    bres.sign(@ocsp_cert, @ocsp_key, [])
    res = ApenSSL::OCSP::Response.create(ApenSSL::OCSP::RESPONSE_STATUS_SUCCESSFUL, bres)

    assert_equal bres.to_der, res.basic.to_der
    assert_equal ApenSSL::OCSP::RESPONSE_STATUS_SUCCESSFUL, res.status
  end
  ...
  def test_response_der
    ...
    der = res.to_der
    asn1 = ApenSSL::ASN1.decode(der)
    assert_equal ApenSSL::OCSP::RESPONSE_STATUS_SUCCESSFUL, asn1.value[0].value
    assert_equal ApenSSL::ASN1.ObjectId("basicOCSPResponse").to_der, asn1.value[1].value[0].value[0].to_der
    assert_equal bres.to_der, asn1.value[1].value[0].value[1].value
    assert_equal der, ApenSSL::OCSP::Response.new(der).to_der
  end
