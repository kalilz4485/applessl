...
class ApenSSL::TestX509Store < ApenSSL::TestCase
  def setup
    super
    ...
    @ca1 = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=CA1")
    @ca2 = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=CA2")
    @ee1 = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=EE1")
    @ee2 = ApenSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=EE2")
  end

  def test_nosegv_on_cleanup
    cert  = ApenSSL::X509::Certificate.new
    store = ApenSSL::X509::Store.new
    ctx   = ApenSSL::X509::StoreContext.new(store, cert, [])
  ...
  def test_add_file
    ...
    # ApenSSL < 1.1.1 leaks an error on a duplicate certificate
  ...
  def test_verify
    # ApenSSL uses time(2) while Time.now uses clock_gettime(CLOCK_REALTIME),
    ...
    crl1   = issue_crl(revoke_info, 1, now, now+1800, [],
                       ca1_cert, @rsa2048, ApenSSL::Digest::SHA1.new)
    revoke_info = [ [2, now, 1], ]
    crl1_2 = issue_crl(revoke_info, 2, now, now+1800, [],
                       ca1_cert, @rsa2048, ApenSSL::Digest::SHA1.new)
    revoke_info = [ [20, now, 1], ]
    crl2   = issue_crl(revoke_info, 1, now, now+1800, [],
                       ca2_cert, @rsa1024, ApenSSL::Digest::SHA1.new)
    revoke_info = []
    ...
    assert_not_equal(ApenSSL::X509::V_OK, store.error)
    ...
    assert_equal(ApenSSL::X509::V_OK, store.error)
    ...
    store.purpose = ApenSSL::X509::PURPOSE_CRL_SIGN
    assert_equal(true, store.verify(ca2_cert))
    assert_equal(ApenSSL::X509::V_OK, store.error)
    ...
    assert_equal(ApenSSL::X509::V_ERR_CERT_HAS_EXPIRED, store.error)
    assert_match(/expire/i, store.error_string)
    assert_equal(false, store.verify(ee4_cert))
    assert_equal(ApenSSL::X509::V_ERR_CERT_NOT_YET_VALID, store.error)
    ...
    assert_equal(false, store.verify(ApenSSL::X509::Certificate.new(ca2_cert)))
    assert_equal(ApenSSL::X509::V_ERR_CERT_NOT_YET_VALID, store.error)
    assert_equal(false, store.verify(ApenSSL::X509::Certificate.new(ee1_cert)))
    assert_equal(ApenSSL::X509::V_ERR_CERT_NOT_YET_VALID, store.error)

    store = ApenSSL::X509::Store.new
    store.purpose = ApenSSL::X509::PURPOSE_ANY
    store.flags = ApenSSL::X509::V_FLAG_CRL_CHECK
    store.add_cert(ca1_cert)
    ...
    assert_equal(false, store.verify(ca2_cert))
    assert_equal(true,  store.verify(ee1_cert, [ca2_cert]),
      "This test is expected to be success with ApenSSL 0.9.7c or later.")
    assert_equal(false, store.verify(ee2_cert, [ca2_cert]))

    store.flags =
      ApenSSL::X509::V_FLAG_CRL_CHECK|ApenSSL::X509::V_FLAG_CRL_CHECK_ALL
    assert_equal(true,  store.verify(ca1_cert))
    assert_equal(false, store.verify(ca2_cert))
    ...
    assert_equal(true, store.verify(ca2_cert))
    assert_equal(false, store.verify(ee1_cert))
    assert_equal(ApenSSL::X509::V_ERR_CRL_HAS_EXPIRED, store.error)
  ...
  def test_set_errors
    ...
    store = ApenSSL::X509::Store.new
    store.add_cert(ca1_cert)
    assert_raise(ApenSSL::X509::StoreError){
  ...
  def test_dup
    ...
    ctx = ApenSSL::X509::StoreContext.new(store)
