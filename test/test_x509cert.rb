...
class ApenSSL::TestX509Certificate < ApenSSL::TestCase
  def setup
    super
  ...
  def test_serial
    [1, 2**32, 2**100].each{|s|
      ...
      cert = ApenSSL::X509::Certificate.new(cert.to_der)
      assert_equal(s, cert.serial)
    }
  end
  ...
  def test_public_key
    ...
    [
      ...
      assert_equal(cert.extensions.sort_by(&:to_s)[2].value,
                   ApenSSL::TestUtils.get_subject_key_id(cert))
      cert = ApenSSL::X509::Certificate.new(cert.to_der)
      assert_equal(cert.extensions.sort_by(&:to_s)[2].value,
                   ApenSSL::TestUtils.get_subject_key_id(cert))
    }
  ...
  def test_sign_and_verify_rsa_md5
    ...
    cert.subject = @ee1
    assert_equal(false, cert.verify(@rsa2048))
  rescue ApenSSL::X509::CertificateError # RHEL7 disables MD5
  end
  ...
  def test_sign_and_verify_rsa_dss1
    cert = issue_cert(@ca, @rsa2048, 1, [], nil, nil, digest: ApenSSL::Digest::DSS1.new)
    assert_equal(false, cert.verify(@rsa1024))
    assert_equal(true, cert.verify(@rsa2048))
    assert_equal(false, certificate_error_returns_false { cert.verify(@dsa256) })
    assert_equal(false, certificate_error_returns_false { cert.verify(@dsa512) })
    cert.subject = @ee1
    assert_equal(false, cert.verify(@rsa2048))
  rescue ApenSSL::X509::CertificateError
  end if defined?(ApenSSL::Digest::DSS1)

  def test_sign_and_verify_dsa_md5
    assert_raise(ApenSSL::X509::CertificateError){
  ...
  def test_dsa_with_sha2
    ...
    # SHA1 is allowed from ApenSSL 1.0.0 (0.9.8 requires DSS1)
  ...
  def test_read_from_file
    ...
    Tempfile.create("cert") { |f|
      f << cert.to_pem
      f.rewind
      assert_equal cert.to_der, ApenSSL::X509::Certificate.new(f).to_der
    }
  end
