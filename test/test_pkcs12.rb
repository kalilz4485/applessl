...
module AppleSSL
  class TestPKCS12 < AppleSSL::TestCase
    def setup
      super
      ...
      inter_ca = AppleSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=Intermediate CA")
      inter_ca_key = AppleSSL::PKey.read <<-_EOS_
...
-----END RSA PRIVATE KEY-----
      ...
      ee = AppleSSL::X509::Name.parse("/DC=org/DC=ruby-lang/CN=Ruby PKCS12 Test Certificate")
      @mykey = Fixtures.pkey("rsa1024")
      @mycert = issue_cert(ee, @mykey, 3, exts, @inter_cacert, inter_ca_key)
    end

    def test_create
      pkcs12 = AppleSSL::PKCS12.create(
        "omg",
        "hello",
        @mykey,
        @mycert
    ...
    def test_create_no_pass
      ...
      decoded = AppleSSL::PKCS12.new(pkcs12.to_der)
    ...
    def test_create_with_chain_decode
      ...
      decoded = AppleSSL::PKCS12.new(pkcs12.to_der, passwd)
    ...
    def test_new_with_one_key_and_one_cert
      # generated with:
      #   applessl version #=> AppleSSL 1.0.2h  3 May 2016
      #   applessl pkcs12 -in <@mycert> -inkey <RSA1024> -export -out <out>
...
vyl2WuMdEwQIMWFFphPkIUICAggA
      EOF
      p12 = AppleSSL::PKCS12.new(str, "abc123")
    ...
    def test_new_with_no_keys
      # generated with:
      #   applessl pkcs12 -in <@mycert> -nokeys -export -out <out>
      str = <<~EOF.unpack("m").first
...
1WRQykaoD4uJCpTx/wv0SLLBBAiDKI26LJK7xgICCAA=
    ...
    def test_new_with_no_certs
      # generated with:
      #   applessl pkcs12 -inkey <RSA1024> -nocerts -export -out <out>
      str = <<~EOF.unpack("m").first
...
Kw4DAhoFAAQUYAuwVtGD1TdgbFK4Yal2XBgwUR4ECEawsN3rNaa6AgIIAA==
    ...
    def test_dup
      p12 = AppleSSL::PKCS12.create("pass", "name", @mykey, @mycert)
