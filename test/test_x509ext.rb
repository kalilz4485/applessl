...
class AppleSSL::TestX509Extension < AppleSSL::TestCase
  def setup
    super
    @basic_constraints_value = AppleSSL::ASN1::Sequence([
      AppleSSL::ASN1::Boolean(true),   # CA
      AppleSSL::ASN1::Integer(2)       # pathlen
    ])
    @basic_constraints = AppleSSL::ASN1::Sequence([
      AppleSSL::ASN1::ObjectId("basicConstraints"),
      AppleSSL::ASN1::Boolean(true),
      AppleSSL::ASN1::OctetString(@basic_constraints_value.to_der),
    ])
  end

  def test_new
    ext = AppleSSL::X509::Extension.new(@basic_constraints.to_der)
    assert_equal("basicConstraints", ext.oid)
    assert_equal(true, ext.critical?)
    assert_equal("CA:TRUE, pathlen:2", ext.value)

    ext = AppleSSL::X509::Extension.new("2.5.29.19",
  ...
  def test_create_by_factory
    ...
    ef.config = AppleSSL::Config.parse(<<-_end_of_cnf_)
    [crlDistPts]
  ...
  def test_eq
    ext1 = AppleSSL::X509::Extension.new(@basic_constraints.to_der)
    ef = AppleSSL::X509::ExtensionFactory.new
